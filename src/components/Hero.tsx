...
const Hero = () => {
  ...
  useGSAP(() => {
    if (containerRef.current && isClient && typeof window !== "undefined") {
      ...
      pinTriggerRef.current = ScrollTrigger.create({
        ...
        pin: true,
        pinSpacing: true, // Add this if your layout shifts vertically when pinning
        markers: true,  
      });
    }
  ...
  return (
    <div className="relative h-dvh w-screen overflow-x-hidden  bg-gradient-to-t from-indigo-500 from-10% via-blue-400 via-30% to-black to-90%">
      ...
      >
        <div
          ref={containerRef}
          className="absolute top-0 left-0 z-10 w-screen h-screen overflow-hidden  fit-stretch cursor-zoom-in"
        >
          <BgHero />
        </div>
        <div className=" relative top-0 left-5">
          ...
          <h2
            id="title"
            className="absolute top-80 left-4 text-2xl z-10 font-bold opacity-0"
          >
            A <span id="developer">developer</span> and{" "}
            <span id="designer" className=" hover:text-red-600">
              Web designer
            </span>
          </h2>
          <p
            id="description"
            className="absolute top-90 left-10 text-xl z-10  text-left text-white italic opacity-0"
          >
